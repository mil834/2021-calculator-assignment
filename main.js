...
function item_input_listener(e) {
	...
	const shops = row.querySelectorAll(".macys, .jcpenney, .tjmaxx"); //capture the shop containers within this row
...
function calculate_totals() {
	...
	item_rows.forEach(function (row) {
		...
		const shops = row.querySelectorAll(".macys, .jcpenney, .tjmaxx");
		...
		shops.forEach(function (shop) {
			...
			if (shop.classList.contains("macys")) { //if it's amazon, increase amazon's total
				macys = macys + total;
			}

			if (shop.classList.contains("jcpenney")) { //if it's peapod, increase peapod's total
				jcpenney = jcpenney + total;
			}

			if (shop.classList.contains("tjmaxx")) { //if it's freshdirect, increase freshdirect's total
				tjmaxx = tjmaxx + total;
			}
		});
	});
	...
	total_row.querySelector('.macys span').innerHTML = round_number(macys); // set the total row's amazon container content to be the round number for amazon's total
	total_row.querySelector('.jcpenney span').innerHTML = round_number(jcpenney); // set the total row's peapod container content to be the round number for peapod's total
	total_row.querySelector('.tjmaxx span').innerHTML = round_number(tjmaxx); // set the total row's freshdirect container content to be the round number for freshdirect's total

	/**
	 * Now let's use conditionals to determine which shop cost least
	 */
	if (macys < jcpenney && macys < tjmaxx) { // if amazon's cost is less than both peapod and freshdirect, amazon is cheapest
		cheapest = 'macys';
	}

	if (jcpenney < macys && jcpenney < tjmaxx) { // if peapod's cost is less than both amazon and freshdirect, peapod is cheapest
		cheapest = 'jcpenney';
	}

	if (tjmaxx < macys && tjmaxx < jcpenney) { // if freshdirect's cost is less than both peapod and amazon, freshdirect is cheapest
		cheapest = 'tjmaxx';
	}
